...
export default function TaskCard({ task, editAction, deleteAction, doneAction }) {
    return (
        <article className="flex items-center p-4 rounded border-[rgba(0,0,0,0.1)] border-1 shadow-xs">
            <p className={`${task.done && "line-through"}`}>{task.desc}</p>
            <div className="ml-auto flex gap-0.5 bg-stone-50 text-stone-500 inset-shadow-2xs p-1 text-center rounded">
                <button className={`p-1 transition-colors hover:bg-green-600 ${task.done && "bg-green-600 text-white"} hover:text-white rounded cursor-pointer`} onClick={() => doneAction(task.id)}><LuSquareCheckBig /></button>
